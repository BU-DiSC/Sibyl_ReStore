...
class TriHybridStorageEnvironment(py_environment.PyEnvironment):
    def __init__(self, hybrid):
        super().__init__()
        self._action_spec = array_spec.BoundedArraySpec(
            shape=(), dtype=np.int32, minimum=0, maximum=1, name='action')  
        self._observation_spec = array_spec.BoundedArraySpec(
            shape=(1,9), dtype=np.float64, minimum=None, maximum=None, name='observation') 
        self._action_values = {0:'slow',1:'fast',2:'mid'}
        self._hybrid = hybrid
    ...
    def _reset(self):
        ...
        self._hybrid._trace_index=0
        self._VBA_prev=0
        self._obs=np.append([self._hybrid._state[0,1:3]],[0,0,0,0,0,0,1])
    ...
    def _step(self, action): 
        ...
        filledPercent_mid = self._hybrid._devices.at["midSSD","Filled"]/self._hybrid._devices.at["midSSD","Capacity"]
        self._obs=np.append([self._obs],[filledPercent_mid])
